...
O coeficiente de correlação de Spearman ($\rho$) avalia a força e a direção da associação entre duas variáveis através de uma relação monotônica. Trata-se de um método não paramétrico, pois ele não exige a suposição de que os dados sigam uma distribuição normal, sendo baseado nos postos dos valores ao invés dos valores brutos [@Spearman_1904].
...
-   Hipótese alternativa ($H_1$): Existe uma associação monotônica entre as variáveis (os postos de uma variável tendem a aumentar ou diminuir sistematicamente em relação aos postos da outra).
...
Os resultados são apresentados no relatório destacando o coeficiente de correlação (ρ), que indica a magnitude da associação, e o p-valor correspondente, onde p < 0.05 é considerado significativo e p < 0.01 é considerado altamente significativo para rejeitar a independência entre as variáveis.
...
# 1. Carregar as matrizes de coeficientes e p-valores
matriz_coef <- read_excel("f2_tabelas/matriz_correlacao.xlsx")
matriz_p <- read_excel("f2_tabelas/matriz_p_valor_correlacao.xlsx")

# 2. Transformar a matriz de coeficientes para formato longo
coef_longo <- matriz_coef %>%
  rename(Variavel1 = 1) %>%
  pivot_longer(cols = -Variavel1, names_to = "Variavel2", values_to = "rho")

# 3. Transformar a matriz de p-valores, unir com os coeficientes e filtrar
tabela_p <- matriz_p %>%
  rename(Variavel1 = 1) %>%
  pivot_longer(cols = -Variavel1, names_to = "Variavel2", values_to = "p_valor") %>%
  
  # Cruzamento com a tabela de coeficientes rho
  left_join(coef_longo, by = c("Variavel1", "Variavel2")) %>%
  
  # Aplicação dos filtros de significância e remoção de duplicados
  filter(p_valor < 0.05, 
         p_valor > 0, 
         Variavel1 < Variavel2) %>%
  
  # Criação das colunas de exibição e classificação
  mutate(
  ...
  # Seleção, ordenação e arredondamento
...
# 4. Gerar a tabela formatada com kableExtra
tabela_p %>%
